...
Twenty packs ship under [`skills/`](../skills). The filesystem is always the source of
...
safety metadata (`write_risk`, `runtime_requirement`, required + optional capability
families, `required_mcp_tools`). Regenerate it with `pnpm skills:catalog`; `check:all`
fails when catalog and frontmatter drift.
...
| [hiss-stock-premium-lp-manager](../skills/hiss-stock-premium-lp-manager/SKILL.md) | Scan Stock-Token premium/discount, preview one-sided USDG range-ladders, prepare UNSIGNED LP positions the user signs. |
