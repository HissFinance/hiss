...
- **Ship agent tooling** — an [MCP server](./docs/mcp.md) exposing 39 read/prepare
...
HISS ships **21 agent skills** — self-contained `SKILL.md` instruction packs that
...
| [**hiss-lighter**](./skills/hiss-lighter/SKILL.md)                                   | READ public Lighter (Robinhood-instance) market data and PREPARE typed **UNSIGNED** order/cancel/modify intents; Robinhood Stock-Tokens are USDG-quoted spot books. HISS never signs, submits, or holds a Lighter key — signing is local-only | Normalized markets/orderbook/depth reads, prepare-only order intents (`signed:false`) + a deterministic `intentHash`      |
...
| [`@hiss-finance/mcp-server`](./docs/mcp.md)         | A local Model Context Protocol server exposing 39 read/prepare tools to any MCP-compatible agent. Read and prepare only — never executes.                            |
...
The server registers **39 tools** (21 read, 18 prepare). Representative tools:
...
| Vaults            | [Overview](./docs/vaults/index.md) · [Create](./docs/vaults/create-a-vault.md) · [Manifest](./docs/vaults/vault-manifest.md) · [Risk fuses](./docs/vaults/risk-fuses.md) · [Verified history & USDG accounting](./docs/vaults/verified-history-and-usdg-accounting.md) · [24/7 architecture](./docs/vaults/24-7-architecture.md) |
| Fees              | [Overview](./docs/fees/index.md) · [Vault fees](./docs/fees/vault-fees.md) · [$HISS token fees](./docs/fees/hiss-token-fees.md) · [Reward flywheel](./docs/fees/reward-flywheel.md) · [Stock-Premium LP management fee](./docs/fees/stock-premium-lp-management-fee.md)                                                          |
